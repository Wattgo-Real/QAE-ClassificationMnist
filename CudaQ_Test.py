...
target_digits = [0, 1]  # Hand written digits to classify.
...
grid_img = torchvision.utils.make_grid(x_train[:10].cpu(), nrow=5, padding=3, normalize=True)
...
for epoch in range(epochs):
    ...
    training_accuracy.append(accuracy_score(y_train, y_hat_train).item())
    ...
    with torch.no_grad():
        ...
        testing_accuracy.append(accuracy_score(y_test, y_hat_test).item())
        testing_cost.append(test_cost.item())

    print("Training [{:.0f}%]\tCost: {:.4f}".format(100.0 * (epoch + 1) / epochs, training_cost[-1]))
